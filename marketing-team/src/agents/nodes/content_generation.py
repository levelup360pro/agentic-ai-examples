"""
Content generation node: Creates/updates content draft using tool results.
...
def content_generation_node(
    ...
    """
    Generate content draft using tool results from state (agentic path).
    ...
    logger.info(
        "Generated draft (length: %d chars) pattern='%s' iteration=%d",
        len(draft_content), pattern, iteration_count
    )
