...
from src.agents.nodes.content_planning import content_planning_node
from src.agents.nodes.content_generation import content_generation_node
from src.agents.nodes.content_evaluation import content_evaluation_node

from src.agents.tools.rag_search import create_rag_search_tool
from src.agents.tools.web_search import create_tavily_search_tool

from src.rag.vector_store import VectorStore
from src.rag.rag_helper import RAGHelper
from src.utils.llm_client import LLMClient
from src.utils.config_loader import load_brand_config
from src.utils.paths import CHROMA_DIR, CONFIG_DIR
...
from src.evaluation.content_evaluator import ContentEvaluator
...
def route_after_content_evaluation(state: ContentGenerationState) -> Literal["content_generation_node", "end"]:
    ...
    Logic:
      ...
      - Else: end
    """
    meets = bool(state.get("meets_quality_threshold", False))
    iteration_count = int(state.get("iteration_count", 0))
    max_iterations = int(state.get("max_iterations", 1))

    logger.info(
        "Eval routing: meets=%s, iteration_count=%d, max_iterations=%d",
        meets, iteration_count, max_iterations
    )
...
def build_content_workflow(brand: str):
    ...
    content_generator = ContentGenerator(
        ...
        content_evaluator=ContentEvaluator(llm_client=completion_client),  # provided to support patterns if used
    )
